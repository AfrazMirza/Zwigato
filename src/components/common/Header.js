...
// ── REDUX SYSTEM HOOKS INTEGRATION ──
...
const Header = ({ title, showBack = false, ShowWishlist = true, ShowCart = true }) => {
  ...
  // ✅ FIXED: Now reading Cart & Wishlist directly from global Redux Store!
      ...
      <View style={styles.container}>
        ...
        <View style={styles.leftRow}>
          {showBack ? (
            ...
            <View style={styles.appIconContainer}>
              <Image source={AppIcon} style={styles.headerAppIcon} resizeMode="contain" />
            </View>
          )}
          <Text style={styles.logoText}>{title || 'Myntra'}</Text>
        </View>
        ...
        <View style={styles.rightRow}>
          ...
          {ShowWishlist === true ? (
            <TouchableOpacity style={styles.iconBtn} onPress={() => router.navigate('/wishlist')}>
              <Ionicons name="heart-outline" size={24} color={COLORS.iconColor} />
              {/* ✅ Optional: Wishlist me items hone par dynamic dot indicator */}
              {favorites && favorites.length > 0 && <View style={styles.dot} />}
            </TouchableOpacity>
          ) : null}
...
const styles = StyleSheet.create({
  safeArea: { backgroundColor: COLORS.white, paddingTop: 40, borderBottomColor: '#E5E5E5', borderBottomWidth: 0.5 },
  container: {
    height: 50,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  leftRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
  appIconContainer: { paddingLeft: 5 },
  headerAppIcon: { width: 32, height: 32 },
  logoText: {
    fontSize: 16,
    fontFamily: BaseFonts.bold,
    color: COLORS.iconColor,
